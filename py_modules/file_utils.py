...
def create_receive_history_entry(
    ...
    Args:
        ...
        manifest: Exact successful file manifest for new receive records
    ...
    if manifest is not None:
        ...
        entry["items"] = manifest
        if destination_id:
            entry["destinationId"] = destination_id
        if destination_name:
            entry["destinationName"] = destination_name
        if destination_path:
            entry["destinationPath"] = destination_path
        if receive_layout:
            entry["receiveLayout"] = receive_layout
        if session_id:
